...
public class BalanceCommand implements CommandExecutor {
	...
	if (args.length == 0) {
		...
		sender.sendMessage("Error: /balance is only available in game. Use /balance <name>");
		return true;
	    }
	    ...
	    if (userFile.exists()) { 
		sender.sendMessage("Balance for offline player " + plugin.getPlayers().getName(targetUUID));
		double total = 0D;
